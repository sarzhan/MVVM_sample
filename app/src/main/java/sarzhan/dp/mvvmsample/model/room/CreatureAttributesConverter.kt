...
import sarzhan.dp.mvvmsample.model.CreatureAttributes
import java.util.*
...
class CreatureAttributesConverter {
    ...
    fun fromCreatureAttributes(attributes: CreatureAttributes?):String? {
        if (attributes != null){
            return String.format(Locale.US, "%d,%d,%d", attributes.intelligense,attributes.srtength, attributes.endurance)
        }
        return null
    }
